...
| `ST` | `1<<4` = 0x10 | Soil temperature (PT1000) | `analog_sensors` (905 Ω divider) | ADC_IN11 — PA15 (morpho **CN7 pin 17**) since r29 | `soil_temp` (18) | `SENS_OK_PT1000` |
...
> ℹ️ `ST` and `T2` **were** mutually exclusive until r28 (the PT1000 divider
> shared PA10/A2 with I²C1 SDA, and BME280 #2's pull-up corrupted it). The
> divider now lives on **A5** (`ENVNODE_RTD_ON_A5 1`), so `{ALL,15}` is a
> valid set again. A node still wired with the divider on A2 must not select
> both.
