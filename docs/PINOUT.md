...
| 6 | Battery voltage | *(resistor divider — not fitted, compiled out: `ENVNODE_BATT_DIVIDER_FITTED 0`)* | — | — | the INA219 is the battery source (D-20); A5/PB13 is free |
| 6b | Battery V + I | INA219 | **I²C2** | PA12/PA11 | addr `0x40` as fitted (init probes `0x45` then `0x40`), R_shunt 0.1 Ω |
| 7 | Soil temperature | **PT1000 RTD**, 905 Ω divider off 3V3 | **ADC_IN11** | PA15 (**ST-morpho CN7 pin 17**) | ratiometric (`R = Rs·c/(4095−c)`); plain analog pin, no muxing (`ENVNODE_RTD_PIN 2`, r29; CN7 pin 15 beside it = SWCLK). *Was A2/PA10 until r28 — there it shared I²C1 SDA and BME280 #2's powered pull-up added ~250 µA, reading 60 °C in a 21 °C room.* MAX31865 dropped 2026-08-13 |
...
`PB12` (D2) · `PB13` (A5, ADC_IN0) · `PA0` · `PA1` ·
`PA8` · `PB15` · `PC0` · `PC6` · `PC1` (D7). `PB10` (D6) became the status
LED on 2026-08-18 (blink language: LOGBOOK Table 9a); `PA15` (morpho CN7
pin 17) became the PT1000 divider input on 2026-08-25 (r29).
