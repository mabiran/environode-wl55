...
void DBG_Init(void)
{
  ...
  /* SW probes
   * NOTE (KoreroNet/CM0+): the HAL_DBGMCU_* low-power-debug helpers exist only
   * for CPU1 (CM4) — CPU2 cannot be debugged in Stop/Standby. They are guarded
   * out here so this compiles on the M0+ core. The DBGMCU low-power-debug
   * configuration, when wanted, is owned by the CM4 application. */
